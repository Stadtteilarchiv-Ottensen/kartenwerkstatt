	...
	for (var key in feature.properties) {
		if((feature.properties[key] != null) && (feature.properties[key] != "")) {
			if(key == "name") {
				...
				popupContent.push("<tr><td colspan=2 style='text-align: center;'><img title='Bild vergrößern' data-bs-toggle='modal' data-bs-target='#" + imageID + "' style='cursor:pointer;' src='" + url_path_friedhof_images + "phoca_thumb_m2_" + feature.properties[key] + "'><br /><span style='font-size:smaller;'>Bildnachweis: " + imagesrcc + "</span></td></tr>");
				 $("#modalsgohere").append("<div class='modal fade' id='" + imageID + "' tabindex='-1' aria-labelledby='Foto' aria-hidden='true'><div class='modal-dialog'><div class='modal-content'><div class='modal-header'><h5 class='modal-title'>" + imagename + "</h5><button type='button' class='btn-close' data-bs-dismiss='modal' aria-label='Schließen'></button></div><div class='modal-body'><p>" + imagedescription + "</p><img style='width:468px;height:auto;' src='" + url_path_friedhof_images + "phoca_thumb_l_" + imagesrc + "'><p style='font-size:smaller;'>Bildnachweis: " + imagesrcc + "</p></div></div></div></div>");
			}
			else if(key == "type") {}
